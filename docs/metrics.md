...
| Name | Type | Description | Data reviews | Extras | Expiration |
| --- | --- | --- | --- | --- | --- |
| startup.timeline.framework_start |[timespan](https://mozilla.github.io/glean/book/user/metrics/timespan.html) |The duration the Android framework takes to start before letting us run code in `*Application.init`. This is calculated from `appInitTimestamp - processStartTimestamp`. Note: while `appInitTimestamp` always returns milliseconds, `processStartTimestamp` may return inconsistent granularities because it is derived from clock ticks. It returns centiseconds on the Pixel 2. These two timestamps' time units are not converted before reaching a result: e.g. on the Pixel 2, this calculation is `milliseconds - centiseconds`.  |[1](TODO)||2020-10-01 |
| startup.timeline.framework_start_error |[event](https://mozilla.github.io/glean/book/user/metrics/event.html) |An error when attempting to record `framework_start`: one or both of the timestamps used returned a negative value.  |[1](TODO)||2020-10-01 |
